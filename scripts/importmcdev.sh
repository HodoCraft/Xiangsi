...
(
  ...
  if [[ "$lastlog" == *"Xiangsi-Extra mc-dev Imports"* ]]; then
    git reset --hard HEAD^
  fi
)

shopt -s nullglob
server_patches=(patches/server/*.patch)
shopt -u nullglob

if [ ${#server_patches[@]} -eq 0 ]; then
  files=""
  nonnms=""
else
  files=$(grep -h "+++ b/src/main/java/net/minecraft/server/" "${server_patches[@]}" | sort | uniq | sed 's/\+\+\+ b\/src\/main\/java\/net\/minecraft\/server\///g' | sed 's/.java//g')
  nonnms=$(grep -h "create mode " "${server_patches[@]}" | grep -Po "src/main/java/net/minecraft/server/(.*?).java" | sort | uniq | sed 's/src\/main\/java\/net\/minecraft\/server\///g' | sed 's/.java//g')
fi
...
(
  ...
  rm -rf nms-patches
  git add src -A
  echo -e "Xiangsi-Extra mc-dev Imports\n\n$MODLOG" | git commit src -F -
)
